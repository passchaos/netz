...
- WebRTC building blocks: STUN, ICE connectivity-check helpers with priority encode/decode,
  ...
  XOR-MAPPED-ADDRESS helpers, ICE candidates with RFC/Pion-compatible candidate type/local/TCP/relay priority, pair-priority, extension lookup/empty-value round-tripping, and candidate-ufrag generation matching helpers, SDP parsing with DTLS fingerprint, DTLS setup role parse/generation helpers,

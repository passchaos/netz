...
- HTTP/2 frame headers with direct parse/preallocated writes and full-frame direct writes, RFC-bounded SETTINGS validation, DATA/HEADERS (including PADDED/PRIORITY self-dependency checks)/PRIORITY/PUSH_PROMISE/CONTINUATION/RST_STREAM payload parsing, RFC 8336 single-probe exact-indexed ORIGIN origin-set state with exact-allocation/empty-payload parse fast paths, allocation-free writes, empty-index membership and duplicate-entry skips, RFC 7838 single-probe exact-indexed ALTSVC connection/stream advertisements with allocation-free writes, identical replacement skips and per-target replacement, and active-stream reset propagation, a bootstrap
...
- QUIC varints with encode length reuse, long-header parsing, stream IDs, transport parameters, and core
  ...
  with version-specific integrity-tag verification, plus CRYPTO stream reassembly with empty-frame/write/consume no-op handling, pre-reserved CRYPTO frame batches, contiguous duplicate fast paths, and duplicate-overlap conflict detection,
  ...
  receive-frame semantic preflight before multi-frame side effects, preallocated single-, double-, triple-varint, fixed PATH, ACK_FREQUENCY, variable-payload, CRYPTO, STREAM, DATAGRAM, NEW_CONNECTION_ID, and close frame writes, and direct adaptive truncated packet-number

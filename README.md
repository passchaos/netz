...
- HTTP/1.1 requests, responses, preallocated request/response/header-block writes, header field syntax validation, chunked transfer decoding with strict no-leading-OWS chunk-size grammar, exact-body allocation, empty-terminator encode/decode fast paths, extension newline rejection, and extension limits, close-delimited and non-chunked-transfer-coded response bodies, runtime chunked
  ...
  byte buffering with exact-message ownership transfer, preallocated runtime header/trailer assembly for persistent connections, method-aware response body
...
- QUIC varints with encode length reuse and preallocated writes, long-header parsing, stream IDs, transport parameters, and core
  ...
  ClientHello/ServerHello/EncryptedExtensions/Finished encoding with preallocated handshake messages, cipher-suite vectors, SNI/ALPN/supported-groups/key-share extension payloads, extension wrappers, and fixed-width integer writes plus parsing,

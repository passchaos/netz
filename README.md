...
- HTTP/3 frame, SETTINGS, DATAGRAM, request/response HEADERS+DATA helpers,
  ...
  plus an indexed generic origin-keyed idle pool with empty-pool miss fast paths, chunked case-folded hashing, and single-lookup release checks/appends for runtime-specific
  ...
  outstanding request tracking uses single-probe client opens/server activation, empty-index lookup skips, and rejects unknown or duplicate completion and
...
- QUIC varints, long-header parsing, stream IDs, transport parameters, and core
  ...
  indexed single-lookup endpoint-level timer scheduler with empty-index lookup skips for multi-connection loops, and
  ...
  validated path MTU,
  an indexed 1-RTT recovery queue with single-lookup group/packet insertion, cached newest retransmission packet numbers, and aggregate queue stats for PTO plus packet-threshold and time-threshold

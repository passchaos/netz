...
- HTTP/3 frame, SETTINGS, DATAGRAM, request/response HEADERS+DATA helpers,
  ...
  an incremental iterator for CONNECT stream data, normalized HTTP/3 origin
  keys for conservative connection-reuse/coalescing policy decisions,
...
- QUIC varints, long-header parsing, stream IDs, transport parameters, and core
  ...
  work with a due-timer service dispatcher for event-loop arming, and
  ...
  key-update counters plus per-stream send/receive stat snapshots for fast
  telemetry loops,

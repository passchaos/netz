...
- QUIC varints with encode length reuse, long-header parsing, stream IDs, transport parameters, and core
  ...
  RFC-defaulted transport-parameter encoding/validation with stack-buffered integer values and preallocated parameter writes (duplicate detection, exact-allocation raw parsing,
  ...
  transport/application CONNECTION_CLOSE state including frame-payload, ACK, ACK_FREQUENCY negotiation, DATAGRAM negotiation, stream-limit/state/flow-control/final-size/data-conflict, server-only frame, and selected semantic error close emission, indexed PATH_CHALLENGE/PATH_RESPONSE validation state with empty-outstanding challenge/response lookup skips, single-lookup duplicate challenge/response suppression, and caller-storage batch drains/sends wired into 1-RTT,

...
- HTTP/3 frame, SETTINGS, DATAGRAM, request/response HEADERS+DATA helpers,
  ...
  static-name lookups,
  CANCEL_PUSH/PUSH_PROMISE/MAX_PUSH_ID/PRIORITY_UPDATE payload codecs with malformed-payload rejection, indexed per-ID control-state lookups, and advertised MAX_PUSH_ID enforcement for received PUSH_PROMISE frames,
  ...
  indexed bounded per-stream request/response reassembly with short-circuit ready polling and targeted ready-stream lookup, preserving interleaved responses and
...
- QUIC varints, long-header parsing, stream IDs, transport parameters, and core
  ...
  packet-number space ACK tracking with bounded duplicate/old packet suppression,
  ACK range semantic validation plus indexed exact sent-packet lookup and single-pass ACK marking,
  ...
  0-RTT replay filters with digest-indexed duplicate checks, explicit expiry

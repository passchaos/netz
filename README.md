...
- HTTP/2 frame headers, RFC-bounded SETTINGS validation, DATA/HEADERS (including PADDED/PRIORITY self-dependency checks)/PRIORITY/PUSH_PROMISE/CONTINUATION/RST_STREAM payload parsing, RFC 8336 exact-indexed ORIGIN origin-set state, RFC 7838 exact-indexed ALTSVC connection/stream advertisements with per-target replacement, and active-stream reset propagation, a bootstrap
  HPACK static/literal encoder-decoder with RFC 7541 Huffman strings, pseudo-header static fast paths and single-pass request/response pseudo-header extraction, plus
  ...
  management including interleaved SETTINGS/PING/WINDOW_UPDATE/PRIORITY handling and compacting interleaved request FIFO reuse, RFC 9218 SETTINGS_NO_RFC7540_PRIORITIES negotiation and client-only PRIORITY_UPDATE signaling with indexed bounded pre-request buffering, cached idle-request pruning, replacement, and promised-push validation, active-stream RST_STREAM guarding, explicit opt-in PUSH_PROMISE promised-request/pushed-response lifecycle with parent/promised-stream validation, indexed reserved/pending push lookups, O(1) cursor FIFO promised-request notification delivery with targeted pending-index repair, and client RST_STREAM(CANCEL) refusal of reserved pushes, GOAWAY propagation during stream reads and post-GOAWAY request suppression/rejection, cached multi-set drain-floor checks, and same-control-stream GOAWAY emission with persistent control-stream offsets, default client server-push opt-out, connection- and stream-level flow-control enforcement including
  ...
  outbound frame splitting plus h2-style CONTINUATION chain flood, stream-id overflow, indexed active-stream tracking, wrong-direction HEADERS, and idle-stream DATA/WINDOW_UPDATE send limits, with DATA sends that wait for WINDOW_UPDATE capacity
  and inbound DATA consumers that account for full padded frame payloads with indexed stream-window state and restore connection/stream receive capacity,
  ...
  validation with pure-digit Content-Length enforcement, indexed response-body semantics retention, HTTP/2 pseudo-header/lowercase field-name/value and control-character validation, `:method` token validation with case-sensitive HEAD/CONNECT/OPTIONS semantics and `:protocol` token validation,
...
- HTTP/3 frame, SETTINGS, DATAGRAM, request/response HEADERS+DATA helpers,
  ...
  static-name lookups with pseudo-header fast paths,
  ...
  provides an indexed FIFO event-style response/reset queue with explicit stream IDs, while
...
- QUIC varints, long-header parsing, stream IDs, transport parameters, and core
  ...
  packet-number space ACK tracking with cached retained-packet counts and bounded duplicate/old packet suppression,
  ACK range semantic validation plus indexed exact sent-packet lookup, cached sortedness for ACK/loss scans, and single-pass ACK marking,
  ...
  bound resumption PSK derivation, indexed bounded owning server ticket storage with cached LRU eviction,
  ...
  indexed endpoint-level timer scheduler for multi-connection loops, and
  ...
  validated path MTU,
  an indexed 1-RTT recovery queue with cached newest retransmission packet numbers and aggregate queue stats for PTO plus packet-threshold and time-threshold
  retransmission of unacknowledged ack-eliciting frame payloads with single-pass
  ACK-range retirement, single-pass sent-packet tracker stats, exact frame wire-length prediction,
  ...
  cache path-validation deadlines, retry timed-out challenges, expose failed

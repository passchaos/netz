...
- HTTP/3 frame, SETTINGS, DATAGRAM, request/response HEADERS+DATA helpers,
  ...
  HTTP/3 pseudo-header/lowercase field-name, `:method` token validation with case-sensitive CONNECT/OPTIONS semantics, `:protocol` token, URI path/scheme/authority, Host/`:authority`, traditional CONNECT body rules, and connection-specific header validation,

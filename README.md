...
- QUIC varints, long-header parsing, stream IDs, transport parameters, and core
  ...
  digest-indexed single-lookup replay filtering with empty-index membership skips and export/restore snapshots, Retry datagram issue/validate helpers, client-side Retry
  ...
  encoding wired into 1-RTT ACK/STREAM exchange, indexed stream send/receive state with single-lookup flow creation and empty-index lookup skips,
  ...
  override,
  flow-control state with zero-byte reserve no-ops for MAX_DATA/MAX_STREAM_DATA/BLOCKED frames wired into

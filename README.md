...
- QUIC varints, long-header parsing, stream IDs, transport parameters, and core
  ...
  1-RTT, transport/application CONNECTION_CLOSE state including frame-payload, stream-limit/state/flow-control/final-size/data-conflict, server-only frame, and selected semantic error close emission, PATH_CHALLENGE/PATH_RESPONSE validation state with duplicate challenge suppression wired into 1-RTT,

...
- HTTP/3 frame, SETTINGS, DATAGRAM, request/response HEADERS+DATA helpers,
  ...
  request/response message decoding with ordered HEADERS/DATA/trailer handling and interim 1xx response skipping and forbidden trailer-field rejection,
...
- MQTT 3.1.1/5 fixed headers, CONNECT/CONNACK with Last Will and
  ...
  DISCONNECT, AUTH enhanced-authentication exchanges, properties, exact packet bounds, minimal remaining length, MQTT UTF-8 string validation, response-topic/topic-name/filter validation, and
...
- WebRTC building blocks: STUN, ICE connectivity-check helpers with
  ...
  RFC 5285 one-byte/two-byte header extension codecs for MID/RID/TWCC/absolute-send-time style metadata, sender/receiver reports plus sender/receiver-report statistics, SDES/CNAME, BYE, and compound RTCP packets, PLI/FIR/REMB/NACK/TWCC feedback and RTP sequence-gap NACK tracking, SRTP/SRTCP NULL_HMAC_SHA1_80 authentication with ROC/SRTCP-index tracking and replay-window checks, SCTP INIT/INIT-ACK/COOKIE-ECHO/COOKIE-ACK, HEARTBEAT/SHUTDOWN lifecycle, ABORT/ERROR causes, RE-CONFIG stream reset, FORWARD-TSN/I-FORWARD-TSN partial-reliability, and DATA/I-DATA/SACK packet parsing/writing, receive-side SACK state generation, DATA/I-DATA fragment reassembly with receive-buffer limits, CRC32C validation, and DCEP DataChannel OPEN/ACK codecs, plus

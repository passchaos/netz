...
- HTTP/1.1 requests, responses, preallocated header-block writes, header field syntax validation, chunked transfer decoding with strict no-leading-OWS chunk-size grammar, empty-terminator encode/decode fast paths, extension newline rejection, and extension limits, close-delimited and non-chunked-transfer-coded response bodies, runtime chunked

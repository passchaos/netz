...
- HTTP/1.1 requests, responses, header field syntax validation, chunked transfer decoding with strict chunk-size grammar and extension limits, close-delimited response bodies, runtime chunked
  ...
  handling for HEAD and successful CONNECT, authority-form CONNECT target validation and CONNECT tunnel helpers, interim 1xx response skipping plus
...
- HTTP/2 frame headers, RFC-bounded SETTINGS validation, DATA/HEADERS (including PADDED/PRIORITY self-dependency checks)/PRIORITY/PUSH_PROMISE/CONTINUATION/RST_STREAM payload parsing and active-stream reset propagation, a bootstrap
  ...
  dynamic-table indexing/size-update state for long-lived runtimes and automatic
  ...
  SETTINGS_INITIAL_WINDOW_SIZE updates and configurable advertisement, configurable SETTINGS_HEADER_TABLE_SIZE/SETTINGS_MAX_CONCURRENT_STREAMS advertisement, SETTINGS_MAX_FRAME_SIZE and SETTINGS_MAX_HEADER_LIST_SIZE validation,
...
- WebRTC building blocks: STUN, ICE connectivity-check helpers with
  ...
  RFC 5285 one-byte/two-byte header extension codecs for MID/RID/TWCC/absolute-send-time style metadata, sender/receiver reports plus sender/receiver-report statistics, SDES/CNAME and compound RTCP packets, PLI/FIR/NACK/TWCC feedback and RTP sequence-gap NACK tracking, SRTP/SRTCP NULL_HMAC_SHA1_80 authentication with ROC/SRTCP-index tracking and replay-window checks, SCTP INIT/INIT-ACK/COOKIE-ECHO/COOKIE-ACK, HEARTBEAT/SHUTDOWN lifecycle, ABORT/ERROR causes, RE-CONFIG stream reset, FORWARD-TSN partial-reliability, and DATA/SACK packet parsing/writing, receive-side SACK state generation, DATA fragment reassembly with receive-buffer limits, CRC32C validation, and DCEP DataChannel OPEN/ACK codecs, plus

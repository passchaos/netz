...
`std.Io.Threaded` per iteration or sent a client-side QUIC application close at
iteration end still timed out after iteration 0, so the stall is not only reuse
of the top-level Io backend or lack of a local close frame. This narrows the
multi-iteration gap to cleanup/reuse across fresh server/client pairs after a
completed large 4-stream transfer, rather than the first transfer itself.

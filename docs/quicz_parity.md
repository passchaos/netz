...
| Loss recovery / PTO / retransmission | Partially covered and improved | `src/quic/recovery.zig`, `src/quic/one_rtt/tests/recovery_flow.zig`; timer-servicing receives drain bounded ACK-driven losses, while a power-of-two recovery payload cache cuts captured 64 MiB H3 cumulative allocation by 16.9% with a 1 MiB idle-memory cap | Add broader loss-rate/reordering and long-run recovery benchmarks; aggressive non-default packet/body sizing still needs separate coverage. |
...
| HTTP/3 real-handshake download and RFC 9218 response scheduling | Covered for current benchmark scale; feature coverage exceeds the local quicz H3 layer | `HandshakeServerSession.sendResponseBodiesPaced` schedules borrowed complete bodies by the request Priority field and live PRIORITY_UPDATE, serializes non-incremental peers, round-robins incremental peers, bypasses flow-blocked urgent streams, and replans after socket-visible packet prefixes; real-handshake tests cover all four behaviors and the benchmark sends `Priority: u=3, i`. `~/Work/quicz/src/h3` has no Priority field, PRIORITY_UPDATE, urgency, or incremental scheduler. The new scheduler completed five 1MiB/four-stream iterations (129.91 MiB/s mean), one 4MiB/four-stream iteration (118.43 MiB/s), one 16MiB/four-stream iteration (115.97 MiB/s), and three 64MiB/four-stream iterations (11.35 MiB/s mean, 3.49% stddev). | Continue apples-to-apples quicz throughput work before making a whole-stack performance claim; investigate the 64MiB workload's lower steady-state rate separately from its now-passing reliability gate. |
| QPACK / Capsule / WebTransport | Covered by modules, datagram and incremental-stream benchmarks, real-handshake bidi/uni streams, caller-buffer reads, packet-sized partial writes/independent FIN, RESET_STREAM/STOP_SENDING, WT_DRAIN_SESSION and WT_CLOSE_SESSION | `bench-http3-qpack`, `bench-http3-capsule`, `bench-webtransport-datagram`, `bench-webtransport-stream`, `run-webtransport-handshake-stream`; modern 0x41 bidi prefix follows wtransport rather than quicz's legacy Session-ID-only codec; session control is split-safe, bounded and cleans associated streams with WT_SESSION_GONE | Add external wtransport/browser interop and larger concurrency/loss evidence. |

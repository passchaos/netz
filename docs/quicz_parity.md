...
| Flow control / stream reset / STOP_SENDING | Covered by implementation, tests, and quic-go process interop | `src/quic/flow_control.zig`, `src/quic/stream.zig`, one_rtt tests, and `zig build interop-quic-quic-go -Doptimize=ReleaseFast`; the external client sends RESET_STREAM error 41, separately observes STOP_SENDING error 42 and returns RESET_STREAM error 42, and transfers/echoes 12 KiB through advertised 8 KiB connection and 2 KiB stream windows | Add cancellation-under-loss coverage and another external peer before generalizing beyond the verified quic-go lifecycle. |
...
4. **Complete external interop matrix**: public H3 fetch is proven against
   ...
   transport parameters. Supporting that standard quic-go client required netz's

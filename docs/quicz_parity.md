...
| QPACK / Capsule / WebTransport | Covered by modules, datagram and incremental-stream benchmarks, real-handshake bidi/uni streams, caller-buffer reads, packet-sized partial writes/independent FIN, transactional cross-stream packet batches, RESET_STREAM/STOP_SENDING, WT_DRAIN_SESSION and WT_CLOSE_SESSION | `bench-http3-qpack`, `bench-http3-capsule`, `bench-webtransport-datagram`, `bench-webtransport-stream -- --streams=4`, `run-webtransport-handshake-stream`; modern 0x41 bidi prefix follows wtransport rather than quicz's legacy Session-ID-only codec; session control is split-safe, bounded and cleans associated streams with WT_SESSION_GONE. Four 1MiB streams completed at 70 MiB/s aggregate through the socket-visible-prefix batch API. | Add external wtransport/browser interop and cancellation-under-loss evidence. |

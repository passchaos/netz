...
| QPACK / Capsule / WebTransport | Covered by modules, datagram and incremental-stream benchmarks, real-handshake bidi/uni streams, caller-buffer reads, RESET_STREAM and STOP_SENDING | `bench-http3-qpack`, `bench-http3-capsule`, `bench-webtransport-datagram`, `bench-webtransport-stream`, `run-webtransport-handshake-stream`; modern 0x41 bidi prefix follows wtransport rather than quicz's legacy Session-ID-only codec | Add external wtransport/browser interop, runtime CLOSE/DRAIN and partial-write return semantics. |

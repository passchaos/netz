...
| QUIC stream-open churn (`quic_bench_hs`) | Covered and faster for the reference reservation workload; on-wire peer-stream renewal is externally verified | `examples/bench_quic_handshake_stream.zig`, `--mode=stream-churn`; CPU-0-pinned netz sustained 680.2-778.0M streams/s versus quicz 48.1-52.0M streams/s, a 13.1-16.2x range. IDs, limit enforcement, both endpoint roles, zero timed-loop allocation, final ID and checksum are validated. The quic-go process gate separately advertises one bidirectional stream and requires completed streams 0, 4, 8, and 12, proving cumulative MAX_STREAMS renewal. | Add high-volume on-wire FIN/reset recycling and concurrency before generalizing this to full stream lifecycle churn. |
...
| Flow control / stream reset / STOP_SENDING | Covered by implementation, tests, and quic-go process interop | `src/quic/flow_control.zig`, `src/quic/stream.zig`, one_rtt tests, and `zig build interop-quic-quic-go -Doptimize=ReleaseFast`; the external client sends RESET_STREAM error 41, separately observes STOP_SENDING error 42 and returns RESET_STREAM error 42, transfers/echoes 12 KiB through advertised 8 KiB connection and 2 KiB stream windows, and advances from stream 0 through 12 under an initial one-stream limit | Add cancellation-under-loss coverage and another external peer before generalizing beyond the verified quic-go lifecycle. |
...
| TLS backend/process interop examples | Covered in both directions through a standalone, certificate-verified quic-go process gate | QUIC handshake tests, `examples/quic_handshake_echo.zig`, `zig build interop-quic-quic-go -Doptimize=ReleaseFast` (bidirectional/unidirectional streams plus quic-go RESET_STREAM, STOP_SENDING, rolling byte-flow-control, and stream-limit renewal probes), HTTP/3 public fetch with `--verify` | Add broader peers and lifecycle-under-loss cases before making a full interoperability claim. |
...
4. **Complete external interop matrix**: public H3 fetch is proven against
   ...
   `hq-interop` ALPN. Two additional fresh connections verify application
   cancellation: quic-go sends RESET_STREAM with error 41 and then completes a
   stream 4 echo; separately, netz sends STOP_SENDING with error 42, observes
   quic-go's required RESET_STREAM response with the same error, and completes
   another stream 4 echo. A fourth client connection transfers and echoes
   12,288 bytes through deliberately advertised 8,192-byte connection and
   2,048-byte stream windows, so completion requires multiple on-wire
   MAX_DATA/MAX_STREAM_DATA credit updates rather than merely parsing the
   transport parameters. A fifth connection advertises one bidirectional
   stream and completes client streams 0, 4, 8, and 12 in sequence; netz only
   returns each MAX_STREAMS slot after the peer receive side is consumed and
   the local send side reaches FIN or RESET, and duplicate release attempts are
   idempotent. A sixth connection sends a client-initiated unidirectional
   stream 2 and requires a netz-initiated unidirectional stream 3 response,
   with exact payloads and FIN validated in both directions. Supporting that
   standard quic-go client required netz's

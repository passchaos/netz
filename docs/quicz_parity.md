...
1. **Large HTTP/3 download reliability**: local real-handshake download works for
   small and 1MiB smoke tests, but larger responses have shown timeouts or
   local queue/recovery sensitivity.  This is the top functionality gap.
...
1. Build a deterministic local test for large HTTP/3 streaming responses that
   drives both endpoints explicitly instead of relying on benchmark thread
   timing.
2. Use that test to fix response-side flow-control/recovery/lifecycle handling.

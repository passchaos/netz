...
| UDP batching / GSO / GRO | Covered through timer-aware QUIC/HTTP/3 paths | Batch/GRO APIs plus single-packet `servicePacket` apply current-key datagrams in place with retained frame scratch; benchmarks cover owning, in-place and GRO paths | Keep H3 GRO opt-in on this small-rmem host and retest on tuned systems. |

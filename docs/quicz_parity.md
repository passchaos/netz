...
| Flow control / stream reset / STOP_SENDING | Covered by implementation, tests, and quic-go process interop | `src/quic/flow_control.zig`, `src/quic/stream.zig`, one_rtt tests, and `zig build interop-quic-quic-go -Doptimize=ReleaseFast`; the external client sends RESET_STREAM error 41, and separately observes STOP_SENDING error 42 and returns RESET_STREAM error 42 before stream 4 completes normally | Add cancellation-under-loss coverage and another external peer before generalizing beyond the verified quic-go lifecycle. |
...
4. **Complete external interop matrix**: public H3 fetch is proven against
   ...
   another stream 4 echo. Supporting that standard quic-go client required netz's
